...
func (r *NilBitsStack) PushAlwaysNil(bts []byte) []byte {
	...
	if r.LifoAlwaysNil == MaxNestedStructPointerDepth {
		panic(fmt.Sprintf("we hit our maximum nested struct-inside-struct depth! you must recompile msgp with github.com/glycerine/truepack/msgp/nilbits.go: MaxNestedStructPointerDepth set to greater than the current value of %v, and then regenerate your msgp Unmarshaling code.", MaxNestedStructPointerDepth))
	}
